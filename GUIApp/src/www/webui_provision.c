...
static UINT write_file(FX_MEDIA * media, CHAR * path, const UCHAR * data, ULONG len)
{
    ...
    /* Create if missing (create is harmless if already created). */
    ...
    /* Truncate to zero then write fresh contents. */
...
static UINT provision_one(FX_MEDIA * media,
                          CHAR * path,
                          const UCHAR * data,
                          ULONG len)
{
...
/* Call once after fx_media_open() succeeds and before starting the HTTP server. */
UINT webui_provision_to_usb(FX_MEDIA * media)
{
    ...
    st = provision_one(media, (CHAR *) WEBUI_ROOT_DIR "index.html",
                       WEB_INDEX_HTML, WEB_INDEX_HTML_LEN);
    if (st != FX_SUCCESS) return st;

    st = provision_one(media, (CHAR *) WEBUI_ROOT_DIR "app.html",
                       WEB_APP_HTML, WEB_APP_HTML_LEN);
    if (st != FX_SUCCESS) return st;

    st = provision_one(media, (CHAR *) WEBUI_ROOT_DIR "file_manager.js",
                       WEB_FILE_MANAGER_JS, WEB_FILE_MANAGER_JS_LEN);
    if (st != FX_SUCCESS) return st;
