...
/*LDRA_NOANALYSIS This function is declared externally from GUIX library file */
extern VOID (*_gx_system_memory_free)(VOID * );
/*LDRA_ANALYSIS */
...
#if defined(GX_SOFTWARE_DECODER_SUPPORT)
/*******************************************************************************************************************//**
 * @brief  GUIX display driver for Synergy, png draw function with D/AVE 2D acceleration enabled.
 * This function is called by GUIX if software PNG decoder is enabled.
 * @param   context[in]         Pointer to a GUIX draw context
 * @param   xpos[in]            x axis position of a pixelmap in pixel unit
 * @param   ypos[in]            y axis position of a pixelmap in pixel unit
 * @param   pixelmap[in]        Pointer to a pixelmap
...
VOID _gx_dave2d_png_draw(GX_DRAW_CONTEXT *context, INT xpos, INT ypos, GX_PIXELMAP *pixelmap)
{
    ...
    GX_PIXELMAP     pic_outmap = { .gx_pixelmap_data = (GX_CONST GX_UBYTE *) GX_NULL,
    		                       .gx_pixelmap_aux_data = (GX_CONST GX_UBYTE *) GX_NULL};

    gx_image_reader_create(&image_reader,
                            pixelmap -> gx_pixelmap_data,
                            (INT)(pixelmap -> gx_pixelmap_data_size),
                            (UCHAR) GX_COLOR_FORMAT_32ARGB, (UCHAR) GX_IMAGE_READER_MODE_ALPHA);

    if (gx_image_reader_start(&image_reader, &pic_outmap) == (UINT) GX_SUCCESS)
    {
        pixelmap -> gx_pixelmap_flags |= (UCHAR) GX_PIXELMAP_DYNAMICALLY_ALLOCATED;
        context -> gx_draw_context_display -> gx_display_driver_pixelmap_draw(context, xpos, ypos, &pic_outmap);

        /*LDRA_INSPECTED 496 S declared externally. */
