...
/***********************************************************************************************************************
 * Copyright [2017-2025] Renesas Electronics Corporation and/or its licensors. All Rights Reserved.
 * 
...
/*******************************************************************************************************************//**
 * @brief  This function schedules new transfers from isochronous list.
 *
...
VOID  ux_hcd_synergy_iso_schedule(UX_HCD_SYNERGY *hcd_synergy)
{
    ...
    /* Get the pointer to the current ED in the isochronous list.  */
    ed =  hcd_synergy -> ux_hcd_synergy_iso_current_ed;

    /* Check if there is any EDs candidate in the isochronous list.  */
    if (ed == UX_NULL)
    {
        /* Check if there is any ED in the isochronous list. If none, nothing to do.  */
        if (hcd_synergy -> ux_hcd_synergy_iso_head_ed == UX_NULL)
        {
            return;
        }
        else
        {
            /* We use the first ED of the isochronous list.  */
            ed = hcd_synergy -> ux_hcd_synergy_iso_head_ed;
        }
    }
    ...
    do
    {
        ...
        if (   (ed -> ux_synergy_ed_tail_td != ed -> ux_synergy_ed_head_td)
            ...
            status =  ux_hcd_synergy_iso_td_add(hcd_synergy, ed);
            ...
            if (status == (UINT)UX_SUCCESS)
            {
                ...
                if (ed -> ux_synergy_ed_next_ed == UX_NULL)
                {
                    ...
                    hcd_synergy -> ux_hcd_synergy_iso_current_ed =  hcd_synergy -> ux_hcd_synergy_iso_head_ed;
                }
                else
                {
                    /* This is not the end, so use the next ED as the next candidate.  */
                    hcd_synergy -> ux_hcd_synergy_iso_current_ed =  ed -> ux_synergy_ed_next_ed;
                }
            }
        }
